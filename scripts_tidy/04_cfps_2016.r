## ------------------------------------------------------------------------
##
## Script name: 04_cfps_2016.r
## Purpose: Clean CFPS 2016 data
## Author: Yanwen Wang
...
cfps_2016_adult <- read_stata("data/raw/cfps_2016_adult.dta")
cfps_2016_family <- read_stata("data/raw/cfps_2016_family.dta")
cfps_2016_roster <- read_stata("data/raw/cfps_2016_roster.dta")

# 2 Individual variables --------------------------------------------------

# Number of children (alive)
n_children_df <- cfps_2016_roster %>%
  select(
    pid,
    starts_with("alive_a16_c")
  ) %>%
  mutate(across(where(is.numeric), ~ ifelse(. < 0, 0, .))) %>%
  mutate(n_children = rowSums(select(., starts_with("alive_a16_c")))) %>%
  select(pid, n_children)

# Information from the roster file
demography_df <- cfps_2016_roster %>%
  # ID
  mutate(
  ...
  # Hukou
  mutate(
    hukou_a16_p = ifelse(hukou_a16_p < 0, NA_real_, hukou_a16_p),
    hukou = case_when(
      hukou_a16_p == 1 ~ "agri",
      hukou_a16_p == 3 ~ "non-agri",
      TRUE ~ NA_character_
    )
  ) %>%
  # Migrant status
  mutate(
    outpers_where16_p = ifelse(
      outpers_where16_p < 0,
      NA_real_,
      outpers_where16_p
    ),
    migrant = case_when(
      outpers_where16_p >= 4 ~ 1,
      TRUE ~ 0
    )
  ) %>%
  # Marital status
  mutate(
    tb3_a16_p = ifelse(tb3_a16_p < 0, NA_real_, tb3_a16_p),
    marital = case_when(
      tb3_a16_p == 1 ~ "never-married",
      tb3_a16_p == 2 ~ "married",
      tb3_a16_p == 3 ~ "cohabiting",
      tb3_a16_p == 4 ~ "divorced",
      tb3_a16_p == 5 ~ "widowed",
      TRUE ~ NA_character_
    ),
  ...
  # Education
  mutate(
    tb4_a16_p = ifelse(tb4_a16_p < 0, NA_real_, tb4_a16_p),
    educ = case_when(
      tb4_a16_p == 1 ~ "primary or less",
      tb4_a16_p == 2 ~ "primary or less",
      tb4_a16_p == 3 ~ "middle school",
      tb4_a16_p == 4 ~ "high school",
      tb4_a16_p == 5 ~ "college or higher",
      tb4_a16_p == 6 ~ "college or higher",
      tb4_a16_p == 7 ~ "college or higher",
      tb4_a16_p == 8 ~ "college or higher",
      TRUE ~ NA_character_
    )
  ) %>%
  select(
...
individual_df <- cfps_2016_adult %>%
  ...
  mutate(
    weight = rswt_natcs16,
    weight_panel = rswt_natpn1016
  ) %>%
  # Province and urban
  mutate(
    province = zap_labels(provcd16),
    urban = ifelse(urban16 < 0, NA_real_, urban16),
  ) %>%
  # Income
  mutate(
...
household_df <- cfps_2016_family %>%
  ...
  # Household size
  mutate(
    hh_size = ifelse(familysize16 < 0, NA_real_, familysize16)
  ) %>%
  # Household income
  mutate(
